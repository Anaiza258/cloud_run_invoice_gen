const BACKEND_URL = "https://cloud-run-invoice-gen-1000346396091.europe-west1.run.app";
const FRONTEND_URL = window.location.origin;  // Firebase hosting domain, auto
...
//  *********** go to tool page  ********
  ...
  async function goToTool() {
    ...
    if (user) {
      // User is signed in
      window.location.href = `${FRONTEND_URL}/invoice_tool`;  
    } else {
...
document.getElementById("saveInvoiceBtn").addEventListener("click", async function () {
  ...
  showLoader("Saving your invoice...");

  // Create form data from your invoice form
  const form = document.getElementById("invoiceForm");
  const formData = new FormData(form);

  try {
    // 🔹 Send data to backend (Cloud Run)
    const response = await fetch(`${BACKEND_URL}/save_invoice`, {
      method: "POST",
      body: formData
    });
    ...
    if (result.success && result.pdf_preview_url) {
      // ✅ Go to the preview page
      window.location.href = result.pdf_preview_url;
    } else if (result.success && result.pdf_url) {
      // ✅ Or open PDF directly if backend returns pdf_url
